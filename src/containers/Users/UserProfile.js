...
class UserProfile extends React.Component {
  ...
  render() {
    ...
    if (loading) return (<Aux><Loader /></Aux>);

    let role;
    if (user.account_type == 'D') {role = 'Driver'}
    else if (user.account_type == 'S') { role = 'Support Personnel' }
    else if (user.account_type == 'A') { role = 'Admin' }
      ...
      <Aux>
        ...
        <Row style={styles.userProfile}>
          ...
          <div style={styles.userData}>
                <div>Role: {role}</div>
                <div>License number: {user.driver_license_number}</div>
